...
| 顺序 | 阶段 | 命令 | FlyAI | 说明 |
|------|------|------|-------|------|
| **1** | 确认去程 | `npm run skill:outbound` | `search-flight` | 订票后写入 `bookedOutbound` |
| **2** | 确认返程 | `npm run skill:return:flights` | `search-flight` | 订票后写入 `bookedReturn` |
| **3** | 确认计划 | `npm run skill:plan` | — | Plan A/B + `itineraryConstraints` |
| **4** | 确认酒店 | `npm run skill:hotels` | `search-hotel` | 各段 `hotelOverrides.booked` + `skipMonitor` |
| **5** | 待办运维 | `npm run remind:bookings` | **不查** | `config/booking-schedule.json` 门票/预约/路况 |

**机酒全订后**（`feishuTodosOnly`）：不再 fly.ai 查机酒；飞书只推待办 digest，简报无 TOP3 评分明细。

```bash
npm run skill:workflow:status   # 阶段 1–4 进度
npm run skill:workflow          # 顺序执行到酒店
npm run skill:return            # 一次性 2→3→4（去程已订时）
npm run remind:bookings:dry     # 预览阶段 5 每日 digest
npm run remind:bookings         # 推送阶段 5（07:00 cron）
```

详见 `skills/travel-trip-workflow/SKILL.md`（通用）与 `skills/xinjiang-trip-workflow/SKILL.md`（伊犁实例）。
...
| `reports/xinjiang-travel-brief.md` | **行程简报**（阶段 4/5；机酒已订时为待办模式，无 TOP3） |
| `config/booking-schedule.json` | **阶段 5 待办**（门票/预约/路况 + `booked` 状态） |
